...
class LoginFormAuthentificatorAuthenticator extends AbstractFormLoginAuthenticator
{
    ...
    public function getUser($credentials, UserProviderInterface $userProvider)
    {
        ...
        $user = $this->entityManager->getRepository(User::class)->findOneBy(['nomUtilisateur' => $credentials['nomUtilisateur']]);
        ...
        if (!$user->getEtat() == 1) {
            // fail authentication with a custom error
            throw new CustomUserMessageAuthenticationException("Ooops !!! votre compte a été désactivé Merci de contacter votre Administrateur");
        }
